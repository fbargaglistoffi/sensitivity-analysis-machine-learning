...
rm(list=ls())
source("/Volumes/GoogleDrive/My Drive/Research/Financial Literacy/wetransfer-75b189/outliers_bart.R")
source("/Volumes/GoogleDrive/My Drive/Research/Financial Literacy/wetransfer-75b189/sensitivity_bart.R")
library(haven)
library(BART)
library(ggplot2)
library(mlr)
...
knitr::opts_knit$set(root.dir = '/Volumes/GoogleDrive/My Drive/Research/Financial Literacy/wetransfer-75b189')
```
...
The function needs to specify: (i) a matrix of predictors in the training sample (\texttt{x\_train}), (ii) a matrix of predictors in the test sample (\texttt{x\_test}), (iii) a vector of observed values for the financial literacy score in the test sample (\texttt{y\_test}), (iv) a number of burn-in samples (iterations) to be discarded from the computation of the posterior means and standard deviation (\texttt{nburn}), (v) a number of iterations used for the computation (\texttt{nsamp}), (vi) an outlier coefficient (\texttt{outlier_coef}): e.g., if this coefficient is set to 2, it means that the algorithm will consider as outliers those observations with posterior mean larger than two standard deviations from the mean (i.e., observation outside the 95\% confidence intervals for the mean value) and/or two absolute deviations from the median.

The function outputs: (i) observations with posterior predicted financial literacy scores significantly lower (\texttt{low outliers (sd)}) or higher (\texttt{high outliers (sd)}) than the posterior predicted sample mean score (the significance level can be tuned by changing the \texttt{outlier_coef} parameter);
...
The function \texttt{sensitivity_bart} needs to specify: (i) a matrix of predictors in the training sample (\texttt{x\_train}), (ii) a matrix of predictors in the test sample (\texttt{x\_test}), (iii) a vector of observed values for the financial literacy score in the test sample (\texttt{y\_test}), (iv) a number of burn-in samples (iterations) to be discarded from the computation of the posterior means and standard deviation (\texttt{nburn}), (v) a number of iterations used for the computation (\texttt{nsamp}), (vi) the level of significance for computing differences between posterior predicted values (original model vs augmented model): \texttt{alpha}.

The function outputs: (i) \texttt{rmse_original}: estimated RMSE for the original model, (ii) \texttt{rmse_augmented}: estimated RMSE for the augmented model, (iii) \texttt{rsquared_original}: estimated R squared for the original model, (iv) \texttt{rsquared_augmented}: estimated R squared for the augmented model.
